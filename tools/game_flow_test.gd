...
func _run() -> void:
	...
	for case_entry in game.case_catalog:
		...
		game._manga_followup(first_suspect, "timeline")
		if str(game.state.get("partner_choices", {}).get(first_suspect, "")) != "timeline":
			_fail(case_id + " did not remember partner choice")
			return

		game._observe_suspect(first_suspect)
		...
		if int(game.state.loop) != 2:
			...
			return
		if game.state.clues != clues_before_reset:
			_fail(case_id + " lost evidence across loop reset")
			return
		game.overlay.visible = false
